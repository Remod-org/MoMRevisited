...
```
  ...
  - Recommended: Use server.bat from this repo.  Adjust the script as needed for your install path, and start the server ONLY using server.bat.  This includes the injection.
  - OR, run the inject.bat to patch the server.  This should return:
...
Maintain the quotes since the path contains spaces.
  ...
  - Select MemoriesOfMars.exe and press Select - this patches the live game to allow it to connect via console to your server.  There may also be a button labeled "Inject DLL" depending on the version.
...
open 127.0.0.1?accid=ABC123?authticket=ABC123?gender=male?bSpawnBot=false
  ...
  - This mod does replace the function of the back tick key.  For example, to reset your hotbar from build to inventory you may need to remap the key for this.  Normally, tilde ~ may work (Shift-`)
